...
import com.teamsmokeweed.filesinfo.GetFileInfoResponse;
import com.teamsmokeweed.filesinfo.GetFileInfoRowMapper;
...
//        }
    ...
    public int UploadFileToDB(MultipartFile file, int doc_id){
        ...
        Map<String, Object> response = this.jdbcTemplate.queryForMap("SELECT file_upload_id FROM file_upload WHERE doc_id = ? AND file_upload_revision = ? AND file_name = ? AND file_size = ?",
                new Object[]{ doc_id, 1, fileName, fileSize});
        return (Integer) response.get("file_upload_id");
    }
    //oldDocument oleFile
    public void UploadFileToDB(MultipartFile file, int doc_id, int file_upload_id){
        String fileName = file.getOriginalFilename();
        long fileSize = file.getSize();
//        GetFileInfoResponse response = jdbcTemplate.queryForObject("", new Object[]{}, GetFileInfoResponse.class)
    ...
    public List<GetFileInfoResponse> GetFileInfo(int doc_id){

        List<GetFileInfoResponse> list = jdbcTemplate.query("SELECT file_upload_id, doc_id, MAX(file_upload_revision) AS file_upload_revision, GROUP_CONCAT( file_name SEPARATOR '|') AS file_name, MAX(file_upload_date) AS file_upload_date, file_size FROM file_upload WHERE doc_id = ? GROUP BY file_upload_id",
